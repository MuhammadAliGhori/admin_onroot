...
exports.verifyEmail = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: "verified", message: "Email verified successfully" });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
exports.loginUser = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(200)
        .json({ status: false, message: "User not exists against the email" });
    }
...
exports.logoutUser = async (req, res) => {
  try {
    ...
    // Invalidate the token or session (update the user's token data)
    user.token = null; // Set the token to null or mark it as invalid
...
exports.contactusMailSend = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p>
                  You have received a new contact message from your website. Below are the details:
              </p>
              <hr>
