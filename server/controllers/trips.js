...
const createTripsPost = async (req, res) => {
  try {
    const { title, email, sdate, edate, note, userID } = req.body;
    ...
    return res.status(201).json({ status: true, message: "Trip added successfully", data:itinerary });
  } catch (err) {
    if (err.name === "ValidationError") {
      const errorMessages = Object.values(err.errors).map(
        (error) => error.message
...
const getTripsPost = async (req, res) => {
  const { title, email, edate, sdate, userID } = req.query;
...
const updateTripPost = async (req, res) => {
  const tripId = req.params.tripId;
  const { title, email, sdate, edate } = req.body;
...
const updateTrip = async (req, res) => {
  const { tripId, title,email, sdate, edate } = req.body;
  console.log(tripId, title, sdate, edate)
...
const userVisitedCountriesRegions = async (req, res) => {
  try {
    const userId = req.body.userID; // Get the user ID from the query parameter using req.query
    if (!userId) {
      return res.status(200).json({ status: false, message: 'User not provided' });
    }
    ...
    // Create an array to store recommendation Object IDs
    const recommendationIds = [];

    // Iterate through the user's trips and extract recommendation Object IDs from posts
    for (const trip of trips) {
      for (const postId of trip.posts) {
        recommendationIds.push(postId);
      }
    }

    // Find all recommendations with Object IDs stored in recommendationIds array
    ...
    // Iterate through the recommendations to extract city names
    for (const recommendation of recommendations) {
      const location = recommendation.location; // Assuming 'location' is the property in the recommendation object that holds the city name
      if (location) {
        // Check if the city already exists in the visitedCities array
        if (!visitedCities.includes(location)) {
          visitedCities.push(location);
          // You can also extract country names from the recommendations and add them to visitedCountries if needed.
        }
      }
    }

      // Get the total counts
      const totalSimilarCities = visitedCities.length;
      const totalSimilarCountries = visitedCountries.length;
...
const totaleventAgainstTrips = async (req, res) => {
  const { id} = req.body;
  try {
    ...
    const count = await Trips.countDocuments({ posts: id });
    
    // Return 0 if no matching documents found
    return res.status(201).json({
      status: true,
